...
(function() {
    ...
    app.controller('InstallationAreaCtrl', ['$scope', '$rootScope', '$log', 'uiGridConstants', '$aside', 'ebizAlert', 'installation_area',
    function($scope, $rootScope, $log, uiGridConstants, $aside, ebizAlert, installation_area) {

        /* ***************************************************
         * scope variables define
         * ************************************************* */
        $scope.installation_areas = []; // datas
        $scope.installation_area  = {}; // selected data
        /* ***************************************************
        ...
        var gridOptions = {
             data                     : 'installation_areas' // link $scope.installation_areas
            ...
            ,columnDefs : [
                 {name: 'install_area_code'   , field: 'install_area_code'   , displayName: '설치장소코드'              , width: 150, enableCellEdit: false, type: 'string' }
                ,{name: 'install_area_name'   , field: 'install_area_name'   , displayName: '설치장소명'               , width: 150, enableCellEdit: false, type: 'string' , pinnedLeft: true }
                ,{name: 'install_area_name_local', field: 'install_area_name_local', displayName: '설치장소명_지역'            , width: 150, enableCellEdit: false, type: 'string' }
        ...
        gridOptions.onRegisterApi = function(gridApi) {
            ...
            gridApi.selection.on.rowSelectionChanged($scope, function(row) {
                $scope.installation_area = _.extend({},row.entity);      // grid data copy to form data
            });
        ...
        var singleFilter = function(renderableRows){
            ...
            renderableRows.forEach(function(row){
                ...
                [ 'install_area_name', 'install_area_name_local', 'remarks'].forEach(function(field){
        ...
        function reloadData() {
            ...
            installation_area.search()
                .then(function(result) {
                    ...
                    $scope.filterValue   = '';
                    $scope.installation_area  = {};  // init selected data
                    $scope.installation_areas = result;
                });
        };
        ...
        function updateForm() {
            var data = $scope.installation_area;
        ...
        function deleteData() {
            ...
            ebizAlert.confirm({txn_type: data.txn_type},
                function() {
                    installation_area.remove(data.object_id).then(
                        // delete success
                        function(response) {
        ...
        function openAside(position, data) {
            $aside.open({
                templateUrl: 'installation_area_aside.html',
                placement: position,
                size: 'lg',
                backdrop: false,
                scope: $scope,
                controller: 'InstallationAreaAsideCtrl',
                resolve: {
                    installation_area_data: function() {
                        return data;
                    }
                }
            });
...
(function() {
    ...
    app.controller('InstallationAreaAsideCtrl',
    ['$scope', '$http', '$log', '$uibModalInstance', 'ebizAlert', 'installation_area_data', 'installation_area', 'ebizList',
    function($scope, $http, $log, $uibModalInstance, ebizAlert, installation_area_data, installation_area, ebizList) {

        /* ***************************************************
         * scope variables define
         * ************************************************* */
        $scope.store  = installation_area_data;
        $scope.myform = installation_area_data;
        /* ***************************************************
         * main control and setting define
         * ************************************************* */
        ...
        * ------------------------------------------------- */
        ...
        function saveData(form) {
            ...
            if (form.$invalid) {
                ...
                ebizAlert.confirm({txn_type: installation_area_data.txn_type},
                    // ok call back : insert or update data
                    function() {

                        var txn_type = installation_area_data.txn_type;

                        if( txn_type === 'exec_madd') {
                            // insert Data
                            installation_area.insert($scope.myform).then(
                                // insert success
                                function(response) {
                                    $scope.reloadData();
                            ...
                            // update Data
                            installation_area.update(installation_area_data.object_id, $scope.myform).then(
                                // update success
                                function(response) {
                                    ebizAlert.dbOK(txn_type, response);
                                    $uibModalInstance.close();
                    ...
                    { txn_type: installation_area_data.txn_type, timer: 1000 }
                );
            }
