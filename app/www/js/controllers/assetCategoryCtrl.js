...
(function() {
    ...
    app.controller('AssetCategoryCtrl', ['$scope', '$rootScope', '$log', 'uiGridConstants', '$aside', 'ebizAlert', 'asset_category',
    function($scope, $rootScope, $log, uiGridConstants, $aside, ebizAlert, asset_category) {

        /* ***************************************************
         * scope variables define
         * ************************************************* */
        $scope.asset_categorys = []; // datas
        $scope.asset_category  = {}; // selected data
        /* ***************************************************
        ...
        var gridOptions = {
             data                     : 'asset_categorys' // link $scope.asset_categorys
            ...
            ,columnDefs : [
                 {name: 'asset_cat_code'      , field: 'asset_cat_code'      , displayName: '자산분류코드'              , width: 150, enableCellEdit: false, type: 'string' }
                ,{name: 'asset_cat_name'      , field: 'asset_cat_name'      , displayName: '자산분류명'               , width: 150, enableCellEdit: false, type: 'string' , pinnedLeft: true }
                ,{name: 'asset_cat_name_local', field: 'asset_cat_name_local', displayName: '자산분류명_지역'            , width: 150, enableCellEdit: false, type: 'string' }
        ...
        gridOptions.onRegisterApi = function(gridApi) {
            ...
            gridApi.selection.on.rowSelectionChanged($scope, function(row) {
                $scope.asset_category = _.extend({},row.entity);      // grid data copy to form data
            });
        ...
        var singleFilter = function(renderableRows){
            ...
            renderableRows.forEach(function(row){
                ...
                [ 'asset_cat_name', 'asset_cat_name_local', 'remarks'].forEach(function(field){
        ...
        function reloadData() {
            ...
            asset_category.search()
                .then(function(result) {
                    ...
                    $scope.filterValue   = '';
                    $scope.asset_category  = {};  // init selected data
                    $scope.asset_categorys = result;
                });
        };
        ...
        function deleteData() {
            ...
            ebizAlert.confirm({txn_type: data.txn_type},
                function() {
                    asset_category.remove(data.object_id).then(
                        // delete success
                        function(response) {
        ...
        function openAside(position, data) {
            $aside.open({
                templateUrl: 'asset_category_aside.html',
                placement: position,
                size: 'lg',
                backdrop: false,
                scope: $scope,
                controller: 'AssetCategoryAsideCtrl',
                resolve: {
                    asset_category_data: function() {
                        return data;
                    }
                }
            });
...
(function() {
    ...
    app.controller('AssetCategoryAsideCtrl',
    ['$scope', '$http', '$log', '$uibModalInstance', 'ebizAlert', 'asset_category_data', 'asset_category', 'ebizList',
    function($scope, $http, $log, $uibModalInstance, ebizAlert, asset_category_data, asset_category, ebizList) {

        /* ***************************************************
         * scope variables define
         * ************************************************* */
        $scope.store  = asset_category_data;
        $scope.myform = asset_category_data;
        /* ***************************************************
         * main control and setting define
         * ************************************************* */
        ...
        * ------------------------------------------------- */
        ...
        function saveData(form) {
            ...
            if (form.$invalid) {
                ...
                ebizAlert.confirm({txn_type: asset_category_data.txn_type},
                    // ok call back : insert or update data
                    function() {

                        var txn_type = asset_category_data.txn_type;

                        if( txn_type === 'exec_madd') {
                            // insert Data
                            asset_category.insert($scope.myform).then(
                                // insert success
                                function(response) {
                                    $scope.reloadData();
                            ...
                            // update Data
                            asset_category.update(asset_category_data.object_id, $scope.myform).then(
                                // update success
                                function(response) {
                                    ebizAlert.dbOK(txn_type, response);
                                    $uibModalInstance.close();
                    ...
                    { txn_type: asset_category_data.txn_type, timer: 1000 }
                );
            }
