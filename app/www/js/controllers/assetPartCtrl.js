...
(function() {
    ...
    app.controller('AssetPartCtrl', ['$scope', '$rootScope', '$log', 'uiGridConstants', '$aside', 'ebizAlert', 'asset_part',
    function($scope, $rootScope, $log, uiGridConstants, $aside, ebizAlert, asset_part) {

        /* ***************************************************
         * scope variables define
         * ************************************************* */
        $scope.asset_parts = []; // datas
        $scope.asset_part  = {}; // selected data
        /* ***************************************************
        ...
        var gridOptions = {
             data                     : 'asset_parts' // link $scope.asset_parts
            ...
            ,columnDefs : [
                ...
                //,{name: 'asset_part_seq'      , field: 'asset_part_seq'      , displayName: '부속품일련번호'             , width:100, enableCellEdit: false, type: 'number' }
                ,{name: 'part_no'             , field: 'part_no'             , displayName: '부속품목번호'              , width: 150, enableCellEdit: false, type: 'string' , pinnedLeft: true }
                ,{name: 'part_name'           , field: 'part_name'           , displayName: '품명'                  , width: 150, enableCellEdit: false, type: 'string' }
                ,{name: 'part_name_local'     , field: 'part_name_local'     , displayName: '부품명_지역'              , width: 150, enableCellEdit: false, type: 'string' }
                ,{name: 'part_spec'           , field: 'part_spec'           , displayName: '규격'                  , width: 150, enableCellEdit: false, type: 'string' }
                ,{name: 'quantity'            , field: 'quantity'            , displayName: '재고수량'                , width:100, enableCellEdit: false, type: 'number' }
                ,{name: 'unit_price'          , field: 'unit_price'          , displayName: '단가'                  , width:100, enableCellEdit: false, type: 'number' }
        ...
        gridOptions.onRegisterApi = function(gridApi) {
            ...
            gridApi.selection.on.rowSelectionChanged($scope, function(row) {
                $scope.asset_part = _.extend({},row.entity);      // grid data copy to form data
            });
        ...
        var singleFilter = function(renderableRows){
            ...
            renderableRows.forEach(function(row){
                ...
                [ 'part_no', 'part_name', 'part_name_local', 'part_spec', 'file_key', 'remarks'].forEach(function(field){
        ...
        function reloadData() {
            ...
            asset_part.search()
                .then(function(result) {
                    ...
                    $scope.filterValue   = '';
                    $scope.asset_part  = {};  // init selected data
                    $scope.asset_parts = result;
                });
        };
        ...
        function deleteData() {
            ...
            ebizAlert.confirm({txn_type: data.txn_type},
                function() {
                    asset_part.remove(data.object_id).then(
                        // delete success
                        function(response) {
        ...
        function openAside(position, data) {
            $aside.open({
                templateUrl: 'asset_part_aside.html',
                placement: position,
                size: 'lg',
                backdrop: false,
                scope: $scope,
                controller: 'AssetPartAsideCtrl',
                resolve: {
                    asset_part_data: function() {
                        return data;
                    }
                }
            });
...
(function() {
    ...
    app.controller('AssetPartAsideCtrl',
    ['$scope', '$http', '$log', '$uibModalInstance', 'ebizAlert', 'asset_part_data', 'asset_part', 'ebizList',
    function($scope, $http, $log, $uibModalInstance, ebizAlert, asset_part_data, asset_part, ebizList) {

        /* ***************************************************
         * scope variables define
         * ************************************************* */
        $scope.store  = asset_part_data;
        $scope.myform = asset_part_data;
        /* ***************************************************
         * main control and setting define
         * ************************************************* */
        ...
        * ------------------------------------------------- */
        ...
        function saveData(form) {
            ...
            if (form.$invalid) {
                ...
                ebizAlert.confirm({txn_type: asset_part_data.txn_type},
                    // ok call back : insert or update data
                    function() {

                        var txn_type = asset_part_data.txn_type;

                        if( txn_type === 'exec_madd') {
                            // insert Data
                            asset_part.insert($scope.myform).then(
                                // insert success
                                function(response) {
                                    $scope.reloadData();
                            ...
                            // update Data
                            asset_part.update(asset_part_data.object_id, $scope.myform).then(
                                // update success
                                function(response) {
                                    ebizAlert.dbOK(txn_type, response);
                                    $uibModalInstance.close();
                    ...
                    { txn_type: asset_part_data.txn_type, timer: 1000 }
                );
            }
        ...
        * ------------------------------------------------- */
        ...
        $scope.asset_common_part_list    = ebizList.list('asset_common_part'   );    //부속품목번호
