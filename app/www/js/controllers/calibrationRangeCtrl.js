...
(function() {
    ...
    app.controller('CalibrationRangeCtrl', ['$scope', '$rootScope', '$log', 'uiGridConstants', '$aside', 'ebizAlert', 'calibration_range',
    function($scope, $rootScope, $log, uiGridConstants, $aside, ebizAlert, calibration_range) {

        /* ***************************************************
         * scope variables define
         * ************************************************* */
        $scope.calibration_ranges = []; // datas
        $scope.calibration_range  = {}; // selected data
        /* ***************************************************
        ...
        var gridOptions = {
             data                     : 'calibration_ranges' // link $scope.calibration_ranges
            ...
            ,columnDefs : [
                ...
                ,{name: 'calibration_value'   , field: 'calibration_value'   , displayName: '검교정측정수식'             , width: 150, enableCellEdit: false, type: 'string' , pinnedLeft: true }
        ...
        gridOptions.onRegisterApi = function(gridApi) {
            ...
            gridApi.selection.on.rowSelectionChanged($scope, function(row) {
                $scope.calibration_range = _.extend({},row.entity);      // grid data copy to form data
            });
        ...
        var singleFilter = function(renderableRows){
            ...
            renderableRows.forEach(function(row){
                ...
                [ 'calibration_value', 'remarks'].forEach(function(field){
        ...
        function reloadData() {
            ...
            calibration_range.search()
                .then(function(result) {
                    ...
                    $scope.filterValue   = '';
                    $scope.calibration_range  = {};  // init selected data
                    $scope.calibration_ranges = result;
                });
        };
        ...
        function updateForm() {
            var data = $scope.calibration_range;
        ...
        function deleteData() {
            ...
            ebizAlert.confirm({txn_type: data.txn_type},
                function() {
                    calibration_range.remove(data.object_id).then(
                        // delete success
                        function(response) {
        ...
        function openAside(position, data) {
            $aside.open({
                templateUrl: 'calibration_range_aside.html',
                placement: position,
                size: 'lg',
                backdrop: false,
                scope: $scope,
                controller: 'CalibrationRangeAsideCtrl',
                resolve: {
                    calibration_range_data: function() {
                        return data;
                    }
                }
            });
...
(function() {
    ...
    app.controller('CalibrationRangeAsideCtrl',
    ['$scope', '$http', '$log', '$uibModalInstance', 'ebizAlert', 'calibration_range_data', 'calibration_range', 'ebizList',
    function($scope, $http, $log, $uibModalInstance, ebizAlert, calibration_range_data, calibration_range, ebizList) {

        /* ***************************************************
         * scope variables define
         * ************************************************* */
        $scope.store  = calibration_range_data;
        $scope.myform = calibration_range_data;
        /* ***************************************************
         * main control and setting define
         * ************************************************* */
        ...
        * ------------------------------------------------- */
        ...
        function saveData(form) {
            ...
            if (form.$invalid) {
                ...
                ebizAlert.confirm({txn_type: calibration_range_data.txn_type},
                    // ok call back : insert or update data
                    function() {

                        var txn_type = calibration_range_data.txn_type;

                        if( txn_type === 'exec_madd') {
                            // insert Data
                            calibration_range.insert($scope.myform).then(
                                // insert success
                                function(response) {
                                    $scope.reloadData();
                            ...
                            // update Data
                            calibration_range.update(calibration_range_data.object_id, $scope.myform).then(
                                // update success
                                function(response) {
                                    ebizAlert.dbOK(txn_type, response);
                                    $uibModalInstance.close();
                    ...
                    { txn_type: calibration_range_data.txn_type, timer: 1000 }
                );
            }
