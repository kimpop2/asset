...
.service('auth', function($q, $http, AUTH_PATH) {
    ...
    function storeUserAuthToken(token) {
        ...
        setCookie(LOCAL_TOKEN_KEY, token, 1); //1day
        useAuthToken(token);
    }
    ...
    var login = function(user) {
        return $q(function(resolve, reject) {
            ...
            $http.post(AUTH_PATH.url + '/login', user).then(function(result) {
                if (result.data.success) {
                    login_user.user_id = user.empno;
                    storeUserAuthToken(result.data.token);
                    resolve(result.data.msg, user);
                } else {
                    login_user = null;
                    reject(result.data.msg);
                }
